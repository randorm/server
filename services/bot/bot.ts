  ...
  if (
    ...
    if (ctx.chat?.id && ctx.session.lastBotMessageId) {
      if (ctx.session.registrationStep === RegistrationStep.Name) {
        await ctx.state.bot.api.editMessageText(
          ctx.chat.id,
          ctx.session.lastBotMessageId,
          "Enter your name and surname like _Name Surname_",
          { parse_mode: "MarkdownV2" },
        );
        await ctx.state.bot.api.editMessageReplyMarkup(
          ctx.chat.id,
          ctx.session.lastBotMessageId,
          {
            reply_markup: {
    ...
    await ctx.state.bot.api.editMessageText(
      ctx.chat.id,
      ctx.session.lastBotMessageId,
      "Confirmed! Now you are registered. Soon will add the ability to edit your data? Now use /profile.",
    );
    ctx.session.registrationStep = RegistrationStep.Next;
    ctx.session.previousStep = undefined;
    const model: ProfileModel = {
      ...
      gender: genderValidation(ctx.session.userData.gender),
